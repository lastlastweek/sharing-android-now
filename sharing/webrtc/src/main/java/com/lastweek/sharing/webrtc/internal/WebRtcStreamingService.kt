package com.lastweek.sharing.webrtc.internal
...
import com.lastweek.sharing.common.analytics.EntryPoint
import com.lastweek.sharing.common.analytics.StartFailGroup
import com.lastweek.sharing.common.analytics.StreamMode
import com.lastweek.sharing.common.analytics.StreamingAnalytics
import com.lastweek.sharing.common.analytics.StreamingSessionAnalyticsTracker
import com.lastweek.sharing.common.getLog
import com.lastweek.sharing.common.getVersionName
import com.lastweek.sharing.common.module.ProjectionCoordinator
import com.lastweek.sharing.webrtc.R
import com.lastweek.sharing.webrtc.WebRtcModuleService
import com.lastweek.sharing.webrtc.settings.WebRtcSettings
import com.lastweek.sharing.webrtc.ui.WebRtcError
import com.lastweek.sharing.webrtc.ui.WebRtcState
import com.lastweek.sharing.webrtc.ui.isExpectedEnvironmentIssue
    ...
    private suspend fun processEvent(event: WebRtcEvent) {
        when (event) {
            ...
            is InternalEvent.GetNonce -> {
                if (!destroyPending) {
                    ...
                    signaling = SocketSignaling(environment, okHttpClient, ssEventListener, passwordVerifier)
                        .apply { openSocketNoToken(versionName) }
                    return
                }
                if (destroyPending) {
            ...
            is InternalEvent.StreamCreate -> {
                ...
                val currentStreamId = StreamId("91558396")
//                val currentStreamId = StreamId(webRtcSettings.data.value.lastStreamId)
            ...
            is InternalEvent.StreamCreated -> {
                ...
                if (currentStreamPassword.isEmpty()) currentStreamPassword = StreamPassword.testPassword()
